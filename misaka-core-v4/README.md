...
cd misaka-public-node-macos-arm64
```
...
> **重要**: `--seeds` と `--seed-pubkeys` を指定しないとソロモードになり、テストネットに接続されません。

```bash
export MISAKA_RPC_AUTH_MODE=open   # ローカル開発用 (本番では API Key を設定)

# seed-pubkeys.txt から公開鍵を読み込み (コメント・空行を除去)
SEED_PK=$(grep '^0x' ./config/seed-pubkeys.txt)
...
./misaka-node \
  ...
  --chain-id 2 \
  --rpc-port 3001 \
  --seeds 133.167.126.51:16110 \
  --seed-pubkeys "$SEED_PK"
```
...
| `--seed-pubkeys` | ピアの ML-DSA-65 公開鍵 (PK-pinning 必須) | (seeds と 1:1 対応) |
...
nohup ./misaka-node --config ./config/public-node.toml \
  ...
  --chain-id 2 \
  --seeds 133.167.126.51:16110 \
  --seed-pubkeys "$SEED_PK" > node.log 2>&1 &
...
ExecStart=/home/ubuntu/misaka/misaka-public-node-linux-x86_64/start-public-node.sh
Restart=on-failure
RestartSec=5
LimitNOFILE=65536
...
# ノード起動 (start-public-node.sh が seeds/pubkeys を自動読み込み)
cd /home/ubuntu/misaka/misaka-public-node-linux-x86_64
pm2 start ./start-public-node.sh \
  --name misaka-node \
...
| `SOLO MODE` ログ | `--seeds`/`--seed-pubkeys` 未指定 | 方法 A (`start-public-node.sh`) を使うか、方法 B の通り `--seeds` と `--seed-pubkeys` を指定する |
...
# Faucet (テストネット — API プロキシ経由、レート制限付き)
curl -s http://127.0.0.1:4000/api/v1/faucet/request -X POST \
...
| `/v1/wallet/:address` | GET | 残高 + UTXO 照会 |
| `/v1/wallet/:address/utxos` | GET | UTXO 一覧 |
| `/v1/wallet/:address/history` | GET | TX 履歴 |
