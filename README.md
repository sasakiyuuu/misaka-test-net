...
**現在のノード配布バージョン: v0.8.8**（[`misaka-test-net` Releases](https://github.com/sasakiyuuu/misaka-test-net/releases/latest)）
...
# => {"chainId":2,"version":"0.8.8","topology":"joined",
...
- **Release 公開**: タグ `v*`（例: `v0.8.8`）を push → `[workspace.package] version`（`misaka-core-v4/Cargo.toml`）と一致させる（CI が検証）→ Assets と `SHA256SUMS`（Sigstore 署名付き）が付く
